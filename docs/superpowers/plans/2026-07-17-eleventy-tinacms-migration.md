...
Note: the `"admin": "admin"` passthrough copies a repo-root `admin/` directory that doesn't exist yet at this point in the plan — it's produced later by `npx @tinacms/cli build` in Task 4 (per Task 1's smoke test finding: `tina/config.ts`'s `publicFolder`/`outputFolder` settings, configured in Task 4, make Tina write its static admin bundle to `./admin/` at the repo root, matching the `/admin/` URL every prior CMS iteration on this site has used). Eleventy does not error on a passthrough-copy source that doesn't exist yet — it's a no-op until Task 4 creates that directory, at which point this same config picks it up automatically. Don't remove or "fix" this in Task 3 just because `admin/` isn't present when Task 3 runs its own build check.
...
These exact versions (`@11ty/eleventy@3.1.6`, `tinacms@3.11.0`, `@tinacms/cli@2.5.6`) are what Task 1's smoke test confirmed actually install and work together — use `npm install` (not hand-editing these numbers) so npm resolves whatever the current compatible versions are; update this block to match what actually gets installed if newer versions have since shipped.

- [ ] **Step 6: Update `.gitignore`**

Tina's build output (`admin/`) and generated codegen (`tina/__generated__/`), plus Eleventy's output (`_site/`), are build artifacts — none of them should be committed, same as `node_modules/` already isn't:

```bash
cat >> "c:/Users/Micha/source/repos/uitvaartendan/.gitignore" << 'EOF'

# Eleventy build output
_site/

# TinaCMS build output and codegen (regenerated by `npm run build`)
admin/
tina/__generated__/
EOF
```

(Adjust the path in the `cat >>` command to wherever you're actually running from — see the Context section of your dispatch for the correct working directory.)

- [ ] **Step 7: Run the Eleventy build and verify**
...
```bash
git add .eleventy.js src/_data/site.json src/index.njk package.json package-lock.json .gitignore
...
Expected: completes with no schema errors (Task 1 confirmed this exact command works with real credentials and no extra flags — `--skip-cloud-checks`/`--local` were only needed in Task 1's smoke test because it used fake placeholder credentials), produces `admin/` at the repo root (per the `outputFolder`/`publicFolder` config above) containing a static bundle (`admin/index.html` + `admin/assets/*`).
...
Open the local URL it prints (typically `http://localhost:8080/admin/index.html` or similar — use whatever it actually prints), log in with the Tina Cloud account from Task 2, and confirm every section from Step 1 (Bedrijfsgegevens, Menu, Home, Over mij, Diensten with both pakketten and their item lists, Contact, Footer) appears with the correct current values from `src/_data/site.json`. Stop the dev server after confirming (Ctrl+C).
